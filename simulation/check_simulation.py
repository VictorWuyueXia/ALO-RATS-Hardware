...
def check_suite(output):
    ...
    with (output / "negative_checks.log").open("x") as log:
        negative = subprocess.run([sys.executable, "-m", "pytest", "-q", "-p", "no:cacheprovider",
                                    "test_robot_executor.py", "test_simulation_workflow.py",
                                    "test_simulation_acceptance.py", "test_simulation_entrypoints.py"],
                                   cwd=Path(__file__).parent, stdout=log, stderr=subprocess.STDOUT,
...
"""
    ...
    for index, name in enumerate(names):
        ...
        with (output / f"{name}.log").open("x") as log:
            process = subprocess.run([sys.executable, "-c", worker, case, str(output / name)],
                                     cwd=Path(__file__).parent, env={**os.environ, "PYTHONDONTWRITEBYTECODE": "1"},
    ...
    if direct_passed:
        ...
        process = subprocess.run([sys.executable, str(Path(__file__).with_name("run_simulation.py")),

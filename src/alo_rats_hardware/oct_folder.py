...
def read_oct_folder(directory, values):
    """Validate one saved Lumedica folder and return its surface and content identity."""
    directory = Path(directory)
    expected = int(values["expected_b_scans"])
    indexed = []
    for path in directory.glob(values["file_pattern"]):
        numbers = re.findall(r"\d+", path.stem)
        if not numbers:
            raise ValueError(f"OCT B-scan filename has no numeric index: {path.name}")
        indexed.append((int(numbers[-1]), path))
    if not indexed and any(directory.glob("BSCAN-RAW-*.tif")):
        raise ValueError(
            "OCT folder contains raw BSCAN-RAW files; export processed BSCAN-SGL files with SaveQueueImages")
    indexed.sort()
    indices = [index for index, _ in indexed]
    if len(indices) != expected or len(set(indices)) != expected:
        raise ValueError(f"OCT folder must contain exactly {expected} uniquely indexed B-scans")
    if indices != list(range(indices[0], indices[0] + expected)):
        raise ValueError("OCT B-scan indices must be contiguous")
    ...
    if np.median(peaks) < values["surface_threshold_u8"]:
        raise ValueError("OCT median surface signal is below surface_threshold_u8")

    with ThreadPoolExecutor() as pool:
        payloads = list(pool.map(Path.read_bytes, files))
    hashes = []
    digest = sha256()
    for path, payload in zip(files, payloads, strict=True):
        file_hash = sha256(payload).hexdigest()
        hashes.append({"name": path.name, "bytes": len(payload), "sha256": file_hash})
        digest.update(path.name.encode())
        digest.update(bytes.fromhex(file_hash))
    return files, surface_rows, hashes, digest.hexdigest()


def register_oct_folder(directory, site, scan_pose_base_m):
    """Convert one completed folder at its measured tool0 scan pose to the planning lattice."""
    directory = Path(directory)
    _, surface_rows, hashes, scan_id = read_oct_folder(directory, site.oct)
    ...
    planning_from_oct = (np.linalg.inv(site.registration["base_from_planning_m"])
                         @ scan_pose @ site.registration["tcp_from_oct_m"])
    ...
    surface_scan = SurfaceScan(
        ...
        f"Lumedica shared folder {directory}", "mm", "complete_height_field",
    )
    ...
    scan = VolumetricScan(
        *axes, tissue, np.ones(tissue.shape, dtype=bool), scan_id, accepted_at,
        ...
        f"segmented height field from Lumedica shared folder {directory}",
        "mm", "segmented_occupancy_volume",
    )
    manifest = {
        "scan_id": scan_id, "accepted_at_s": accepted_at,
        "source_directory": str(directory), "file_pattern": values["file_pattern"],
...
class OCTFolderAdapter:
    ...
    def acquire(self, command_id, scan_pose_base_m):
        ...
        scan, manifest = register_oct_folder(stable_path, self.site, scan_pose)
        manifest["command_id"] = command_id
        ...
        self.records.event("oct_scan", sequence=self.sequence, scan_id=scan_id,
                           ...
                           occupied_voxels=int(scan.tissue.sum()))

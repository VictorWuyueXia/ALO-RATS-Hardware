"""Calibrated planning/base transforms for the collaborator's negative-Z laser ray."""
...
class BeamCalibration:
    """Map controller actions to laser poses and measured laser poses back to actions."""
    ...
    def __post_init__(self):
        ...
        if not np.isfinite(self.standoff_m) or self.standoff_m <= 0:
            raise ValueError("Laser standoff must be finite and positive")
    ...
    def identity(self):
        return sha256(self.world_from_base_m.tobytes() + self.base_from_planning_m.tobytes()
                      + np.array([self.standoff_m]).tobytes()).hexdigest()
    ...
    def requested_pose(self, action, plane_z_mm):
        """Retain the radial-forward roll convention with the calibrated standoff."""
        reference = np.array([action.x_mm, action.y_mm, plane_z_mm])
        base_point = self.base_from_planning_m @ np.r_[reference / 1000, 1]
        outward = self.base_from_planning_m[:3, :3] @ (-laser_axis(
            action.tilt_x_rad, action.tilt_y_rad))
        ...
        base_pose[:3, 3] = base_point[:3] + self.standoff_m * outward
        return self.world_from_base_m @ base_pose

    def achieved_action(self, world_from_laser_m, plane_z_mm, energy_j):
        """Recover the measured beam intercept without substituting requested coordinates."""
        ...
        if direction[2] >= -1e-8:
            raise ValueError("Measured laser ray is reversed or parallel to the planning plane")
        distance = (plane_z_mm - point[2]) / direction[2]
        if distance <= 0:
            raise ValueError("Laser origin is below the reference-plane intersection")
